---
name: Page Template Spec
about: WordPress Page Template Functional Specification
title: '[Name] Template'
labels: Page Template
assignees: ''
---

## Functionality

- Briefly describe any functional requirements not captured by individual fields

#### Custom Fields

_\* indicates required_

| Field | [Type](https://www.advancedcustomfields.com/resources/#field-types) | Notes |
| --- | --- | --- |
| \*custom_field | text | describe any field value requirements such as: min/max character lengths, media requirements, relationship with other content |

## Wireframe

<details><summary>Details</summary>

[Wireframe](link_to_invision_app)

<img width="400" src="https://www.satyr.io/400x16:9"/>

</details>

## Design

[Comp](link_to_invision_app)

<img width="400" src="https://www.satyr.io/400x16:9"/>

### Related Issue(s)

- include a link to any related issue(s).
