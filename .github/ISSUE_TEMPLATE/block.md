---
name: Block Spec
about: WordPress Gutenberg Block Functional Specification 
title: ''
labels: Block
assignees: ''
---

## Functionality
- Briefly describe any functional requirements not captured by individual fields

#### Custom Fields

_* indicates required_

Field | [Type](https://www.advancedcustomfields.com/resources/#field-types) | Notes
-- | -- | --
*title | text | descibe any field value requirements such as: min/max character lengths, media requirements, relationship with other content

#### Block Settings

Setting | value
|:-- | --
Supported Post Types | page post
Supports Alignment | false
Supports Multiple | true


## Wireframe

<details><summary>Details</summary>

[Wireframe](link_to_invision_app)

<img width="400" src="https://www.satyr.io/400x16:9"/>

</details>

## Design

[Comp](link_to_invision_app)

<img width="400" src="https://www.satyr.io/400x16:9"/>

### Related Issue(s)

- include a link to any related issue(s).
